    ...
    return {
                ...
                $.ajax({
                    ...
                    complete: function (pages) {
                        ...
                        $('#titleContent').html('').html(`
                            <h1 class="d-flex align-items-center text-danger fw-bolder fs-3 my-1 parentTitle">${responseObject.menu_title}</h1>
                        `);
                        resolve(pages);
                    },
        ...
        handleValidation: function (config) {
            ...
            if (config.el != null && (config.setting != null || config.declarative == true)) {
                ...
                let newFields = {};
                $('#' + config.el).find('input[required], textarea[required], select[required]').map((i, e) => {
                    if (e.name != '') {
                        newFields[e.name] = {
                            selector: '',
                            validators: {
                                notEmpty: {
                                    enabled: true,
                                    message: `Input ${(typeof $(e).attr('aliases') == 'undefined') ? e.name : $(e).attr('aliases')} wajib diisi`
                                }
                            }
                        }
                    }
        ...
        getManualBook : function () {
            // window.location.assign(BASE_APP + 'manbook-v1.0.pdf');
            HELPER.downloadFile(BASE_APP + 'manbook-v1.0.pdf', 'manbook-v1.0.pdf')
        },
