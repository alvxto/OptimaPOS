...
use App\Libraries\ExcelConnection;
...
class Excel
{
	...
	*/
	function setPaperSize($value = '')
	{
		...
		$this->objget->getPageSetup()
    		->setPaperSize(self::getPaperSize($explode[0]));
    		if (count($explode)>1)
    		{
    			if ($explode[1] == 'L')
    			{
					$this->objget->getPageSetup()->setOrientation(Worksheet\PageSetup::ORIENTATION_LANDSCAPE);
				}
				else
				{
					$this->objget->getPageSetup()->setOrientation(Worksheet\PageSetup::ORIENTATION_PORTRAIT);
				}
    		}
		return $this;
	}
	...
	*/
	function setShowGridlines($value = '')
	{
		if ($value) 
		{
			$this->objset->setShowGridlines(TRUE);
		}else
		{
			$this->objset->setShowGridlines(FALSE);
		}
		return $this;
	}
		...
		if (!empty($data))
		{
			...
			else
			{
				foreach ($data as $key => $value)
				{
					...
					$cl = $cell.$start_head;
					$this->setStyles($cl,$cssproperties);
				}
			}
	...
	*/
	...
	function getRichTextObject($value='')
	{
		$html = new PhpOffice\PhpSpreadsheet\Helper\Html();
		return $html->toRichTextObject($value);
	}
	...
	*/
	function url($atRoot = FALSE, $atCore = FALSE, $parse = FALSE)
	{
        ...
        {
            $http = isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off' ? 'https' : 'https';
		...
		if (is_array($data)) 
		{
			if (self::countDim($data)>1)
			{
				foreach ($data as $key => $value)
				{
					...
					$cssproperties = array_diff($cssproperties, array(''));
					$this->setStyles($value['cell'],$cssproperties);
					$cssproperties = [];
			...
			else
			{
				...
				$cssproperties = array_diff($cssproperties, array(''));
				$this->setStyles($data['cell'],$cssproperties);	
	...
	*/
	function upload($config = '')
	{
		...
		if($file['tmp_name'])
		{
			...
			{
				die("The filetype you are attempting to upload is not allowed.");
			}
		}
		return $this;
	}
	...
	*/
	 ...
	 * @param array
	*/
    function calculateStyles($style = '')
	{
	...
	function createStyle($cell = '', $css = '')
	{
		if (!empty($cell) && !empty($css))
		{
			...
			if (isset($this->drawImage)) 
			{
				...
				// offset
				if (isset($_css['img-offset-x']) && $_css['img-offset-x'] != '')
				{
					$this->drawImage->setOffsetX((int) $_css['img-offset-x']);
				}
				if (isset($_css['img-offset-y']) && $_css['img-offset-y'] != '')
				{
					$this->drawImage->setOffsetY((int) $_css['img-offset-y']);
				}
			}
			...
			# alignment
				...
				{
					$_style['alignment']['wrapText'] = (($css['nowrap']) ? true : false);
				}
				...
				if (isset($css['indent']) && $css['indent'] != '') {
					$this->setIndent($cell, $css['indent']);
				}
	...
	*/
	...
	function setIndent($cell='', $value='')
	{
		$this->objset->getStyle($cell)->getAlignment()->setIndent($value);
	}
	...
	*/
	...
	function chart($config=[])
	{
		...
		//     Data values
		//     Data Marker
		$values = self::dataSeries($config['values'], $sheet_title ,'Number');
		...
		if (isset($config['showVal']) && $config['showVal']) {
			$layout->setShowVal($config['showVal']);
		}else{
			$layout->setShowVal(true);
		}
		if (isset($config['showPercent']) && $config['showPercent']) {
			$layout->setShowPercent($config['showPercent']);
		}else{
